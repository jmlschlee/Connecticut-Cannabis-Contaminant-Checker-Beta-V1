"""Bundle the engine + V12.1 report program into ONE self-contained file,
CannaScope_CT_Beta_V12_1.py. Embeds the engine modules + OCR worker, PLUS a compressed
...
Re-run after editing cannascope_ct_v12_1.py (or the v4/v5/names/worker modules)."""
...
_RD = 'CannaScope CT Beta V12.1 - Statewide Transparency Reports'
for _alt in ('CannaScope CT Beta V12 - Statewide Transparency Reports', 'CannaScope CT Beta V11.1 - Statewide Transparency Reports'):
...
v9 = open('cannascope_ct_v12_1.py', encoding='utf-8').read()
...
"""
================================================================================
 CannaScope CT Beta V12.1  —  COMPLETE SELF-CONTAINED SINGLE-FILE BUILD
================================================================================
...
TWO REPORTS
  Statewide Transparency Report (whole market):
    python3 CannaScope_CT_Beta_V12_1.py statewide --days 365
  ...
  Personalized Product Concern Report (one product, for a consumer concern):
    python3 CannaScope_CT_Beta_V12_1.py concern --batch <BATCH> --ndc <NDC> --qr <URL>
    python3 CannaScope_CT_Beta_V12_1.py concern --example
  Outputs go to "CannaScope CT Beta V12.1 - Statewide Transparency Reports/" and
...
def _materialize_ocr_worker():
    try:
        p=_os.path.join(_tmp.gettempdir(),"cannascope_v12_1_ocr_worker.py")
...
out = HEADER + body
open('CannaScope_CT_Beta_V12_1.py', 'w', encoding='utf-8').write(out)
print(f'Wrote CannaScope_CT_Beta_V12_1.py ({len(out):,} bytes)')
