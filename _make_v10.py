"""Bundle the engine + V10 report program into one self-contained file,
CannaScope_CT_Beta_V10.py. V10 is already branded, so no rebrand is needed —
this only embeds the engine modules + OCR worker and prepends the loader header.
Re-run after editing cannascope_ct_v10.py (or the v4/v5/names/worker modules)."""
import base64, zlib
def blob(path):
...
v9 = open('cannascope_ct_v10.py',encoding='utf-8').read()
...
"""
================================================================================
 CannaScope CT Beta V10  —  COMPLETE SELF-CONTAINED SINGLE-FILE BUILD
================================================================================
...
HOW TO RUN
  ...
  python3 CannaScope_CT_Beta_V10.py --days 90
...
PATIENT-REPORTED PRODUCT CONCERN (on-demand personalized patient PDF):
  python3 CannaScope_CT_Beta_V10.py patient-concern --example
  python3 CannaScope_CT_Beta_V10.py patient-concern --ndc C0101000538 --batch ABC --qr <url>
  Writes to output/patient_concerns/ (separate from the regular report; never overwrites).

OUTPUT
  Creates "CannaScope CT Beta V10 - Reports/" beside this file: the PDF report, CSV
  exports, registry cache, and source COA PDFs for flagged products. Reports are NEVER
  overwritten:  CannaScope_CT_Beta_V10_Report_<N>_<YYYY_MM_DD_HHMM>.pdf
"""
...
def _materialize_ocr_worker():
    try:
        p=_os.path.join(_tmp.gettempdir(),"cannascope_v10_ocr_worker.py")
...
out=HEADER+body
open('CannaScope_CT_Beta_V10.py','w',encoding='utf-8').write(out)
print(f'Wrote CannaScope_CT_Beta_V10.py ({len(out):,} bytes)')
